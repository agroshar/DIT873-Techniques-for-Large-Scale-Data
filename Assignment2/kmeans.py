...
def kmeans(k, data, nr_iter=100):
    ...
    total_variation = 0.0
    clustering_times, updating_times = [], []
    for j in range(nr_iter):
        ...
        start_clustering_time = time.perf_counter()
        for i in range(N):
            cluster, dist = nearestCentroid(data[i], centroids)
            c[i] = cluster
            cluster_sizes[cluster] += 1
            variation[cluster] += dist ** 2
        clustering_times.append(time.perf_counter() - start_clustering_time)
        ...
        start_updating_time = time.perf_counter()
        for i in range(N):
            centroids[c[i]] += data[i]
        centroids = centroids / cluster_sizes.reshape(-1, 1)
        updating_times.append(time.perf_counter() - start_updating_time)
